...
export async function POST(req: Request) {
  try {
    ...
    const { data: supplierRows } = await supabase.from("suppliers").select("id,name").eq("company_id", companyId).limit(5000);
    const supplierMap = new Map((supplierRows || []).map((s: any) => [String(s.name || "").trim().toLowerCase(), s.id]));
    ...
    const payload = rows
      .map((r: any, idx: number) => {
        ...
        const supplierName = String(row.preferred_supplier_name || row.supplier_name || row.supplier || "").trim();
