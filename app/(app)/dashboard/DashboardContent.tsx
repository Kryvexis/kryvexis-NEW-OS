...
export default async function DashboardContent({
  title = 'Dashboard',
  subtitle = 'Command center: sales, income, and performance.',
  showActions = true,
}: {
  title?: string
  subtitle?: string
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <div className="text-sm kx-muted">{subtitle}</div>
        </div>
        {showActions ? (
          ...
          <div className="mt-4 flex h-40 items-end gap-3">
            {revenueByMonth.map((m) => (
              <div key={m.key} className="flex flex-1 flex-col items-center gap-2">
                <div className="w-full overflow-hidden rounded-2xl bg-[rgba(var(--kx-fg),.06)]">
                  <div
                    className="w-full"
                    style={{
                      height: `${Math.max(6, Math.round((m.value / maxRev) * 140))}px`,
                      background:
                        'linear-gradient(180deg, rgb(var(--kx-accent) / .85), rgb(var(--kx-accent-2) / .55))',
                    }}
                  />
                </div>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          ...
          <div className="mt-3 space-y-2">
            {topClients.map((c) => (
              <Link
                key={c.id}
                href={`/clients/${c.id}`}
                className="flex items-center justify-between rounded-2xl bg-[rgba(var(--kx-fg),.06)] px-3 py-2 transition hover:bg-[rgba(var(--kx-fg),.09)]"
              >
                <span className="text-sm text-[rgba(var(--kx-fg),.84)]">{c.name}</span>
        ...
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <Link href="/invoices" className="kx-button">
              View all
            </Link>
          </div>

          <div className="mt-3 overflow-hidden rounded-2xl bg-[rgba(var(--kx-fg),.06)]">
            <table className="w-full text-sm">
              <thead className="bg-[rgba(var(--kx-fg),.06)] kx-muted">
                <tr>
              ...
              <tbody>
                {outstandingList.map((i: any) => (
                  <tr key={i.id} className="hover:bg-[rgba(var(--kx-fg),.06)]">
                ...
                {!outstandingList.length && (
                  <tr>
                    <td className="px-3 py-3 text-sm kx-muted2" colSpan={3}>
      ...
      <Card>
        ...
        <div className="mt-3 space-y-2">
          {(activity || []).map((a: any) => (
            <div key={a.id} className="flex items-center justify-between rounded-2xl bg-[rgba(var(--kx-fg),.06)] px-3 py-2">
              <div className="text-sm text-[rgba(var(--kx-fg),.84)]">
                {a.action} · <span className="kx-muted2">{a.entity_type}</span>
              </div>
