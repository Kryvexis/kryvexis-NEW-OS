          ...
          render={(inv) => (
            <div className="rounded-2xl border border-[rgba(var(--kx-border),.12)] bg-[rgba(var(--kx-border),.06)] px-3 py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-medium text-[rgba(var(--kx-fg),.92)]/90">{inv.number ?? "—"}</div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm text-[rgba(var(--kx-fg),.92)]/85 tabular-nums">{fmtZar(Number(inv.total ?? 0))}</div>
                  <div className="text-xs kx-muted tabular-nums">Bal: {fmtZar(Number(inv.balance_due ?? 0))}</div>
