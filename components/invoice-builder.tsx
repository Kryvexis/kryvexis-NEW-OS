...
export default function InvoiceBuilder({ clients, products }: { clients: Client[]; products: Product[] }) {
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="kx-card p-4 md:col-span-2">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="text-xs text-white/55">Draft · balance starts at total</div>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="block">
                ...
                <input className="kx-input" type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
              </label>
            </div>
          </div>
          ...
          <div className="mt-5">
                  ...
                  {items.map((it, idx) => {
                    ...
                    return (
                      <tr key={idx} className="border-t border-white/10">
                        ...
                        <td className="px-3 py-2 text-right">
                          <input className="kx-input text-right" type="number" step="0.01" value={it.discount} onChange={(e) => updateItem(idx, { discount: Number(e.target.value || 0) })} />
                        </td>
                        <td className="px-3 py-2 text-right">
                        ...
                        <td className="px-3 py-2 text-right">
                          <button className="kx-button" onClick={() => setItems((prev) => prev.filter((_, i) => i !== idx))} disabled={items.length <= 1} title="Remove">
                            ✕
                          </button>
                        </td>
                      </tr>
            ...
            <div className="mt-3">
              <button className="kx-button" onClick={() => setItems((prev) => [...prev, { product_id: null, description: 'Item', qty: 1, unit_price: 0, discount: 0, tax_rate: 0.15 }])}>
                + Add line
              </button>
            </div>
          </div>
        ...
        <div className="kx-card p-4 h-fit">
          ...
          <div className="mt-3 space-y-2 text-sm">
            ...
            <div className="text-xs text-white/55 pt-2">Balance due starts at total. Record payments on the invoice page.</div>
          </div>
        </div>
      </div>
    </div>
