    ...
    <div className="grid gap-3">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="grid gap-1">
          <div className="text-xs kx-muted2 uppercase tracking-wider">Search</div>
          <input
          ...
          />
          <div className="text-[11px] text-[rgba(var(--kx-fg),.92)]/45">{filtered.length} match{filtered.length === 1 ? "" : "es"}</div>
        </div>
      ...
      <div className="grid gap-2">
        ...
        {!visible.length && <div className="text-sm kx-muted2">{emptyText}</div>}
      </div>
    </div>
  );
}
