...
export default function MobileTabBar() {
  ...
  return (
    <div className={clsx("fixed inset-x-0 bottom-0 z-50 transition-transform duration-200", hidden ? "translate-y-full" : "translate-y-0")} style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        ...
        <div className="relative rounded-2xl border border-black/5 bg-white/85 shadow-lg backdrop-blur dark:border-white/10 dark:bg-zinc-900/80">
          ...
          <nav className="grid grid-cols-5 gap-1 px-2 pt-4">
            {tabs.map((t) => {
              ...
              return (
                <Link key={t.href} href={t.href} className={clsx("flex flex-col items-center justify-center gap-1 rounded-xl px-2 py-2 text-[11px] font-medium", active ? "text-emerald-700 dark:text-emerald-300" : "text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white")}>
