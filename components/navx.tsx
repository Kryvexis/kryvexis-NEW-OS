...
export function NavIcon({ name }: { name: 'sales' | 'buyers' | 'accounting' | 'operations' | 'insights' | 'settings' | 'help' | 'accountCenter' | 'upload' }) {
  ...
  switch (name) {
    ...
    case 'buyers':
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M6 7h14l-1.5 8.5a2 2 0 0 1-2 1.6H9a2 2 0 0 1-2-1.6L5.5 5H3" stroke="currentColor" strokeWidth="1.5" opacity="0.9" strokeLinecap="round" strokeLinejoin="round"/>
          <circle cx="10" cy="20" r="1.4" fill="currentColor" opacity="0.9"/>
          <circle cx="17" cy="20" r="1.4" fill="currentColor" opacity="0.9"/>
        </svg>
      )
    case 'accounting':
      return (
...
export function Sidebar({ userEmail, workspaceName, role }: { userEmail?: string; workspaceName?: string; role: UserRole }) {
  ...
  return (
    <aside
      ...
      style={{
        background:
          'linear-gradient(180deg, rgb(var(--kx-accent) / 0.14) 0%, rgb(var(--kx-accent) / 0.06) 16%, #08111f 40%, #08111f 70%, #060d18 100%)',
    ...
    >
      ...
      <nav className={'space-y-1 px-3 pb-2'} style={{ color: 'rgba(255,255,255,0.92)' }}>
      ...
      <nav className={'space-y-1 px-3 pt-2 pb-3'} style={{ color: 'rgba(255,255,255,0.92)' }}>
      ...
      {userEmail && (
        <div className="px-5 py-4" style={{ color: 'rgba(255,255,255,0.90)' }}>
          <div className="flex items-center justify-between gap-3">
            ...
            {canManageUsers(role) && <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs" style={{ background: 'rgb(var(--kx-accent) / 0.16)', color: 'rgba(255,255,255,0.92)', border: '1px solid rgb(var(--kx-accent) / 0.28)' }}>Admin</span>}
          </div>
        </div>
      )}
    </aside>
