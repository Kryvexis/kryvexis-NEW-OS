...
export function NavIcon({ name }: { name: NavIconName }) {
  ...
  switch (name) {
    ...
    case 'operations':
      return <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M7 7h10v10H7V7Z" stroke="currentColor" strokeWidth="1.5" opacity="0.9" /><path d="M9 3h6v4H9V3Z" stroke="currentColor" strokeWidth="1.5" opacity="0.9" /></svg>
    case 'insights':
      return <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M5 20V4" stroke="currentColor" strokeWidth="1.5" opacity="0.35" /><path d="M5 18h15" stroke="currentColor" strokeWidth="1.5" opacity="0.35" /><path d="M8 16v-5M12 16v-8M16 16v-3" stroke="currentColor" strokeWidth="1.6" opacity="0.9" strokeLinecap="round" /></svg>
    case 'accountCenter':
  ...
  return (
    <aside className={'hidden md:flex md:flex-col ' + widthCls} style={{ background: 'linear-gradient(180deg, rgb(var(--kx-accent) / 0.24) 0%, rgb(var(--kx-accent) / 0.10) 18%, #0b1220 40%, #0a1628 70%, #081324 100%)', boxShadow: 'var(--kx-shadow-sidebar)', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div className={'flex items-start justify-between gap-3'}>
          <div className={'flex flex-col'}>
            <div className="flex items-center gap-2">
              <Image src="/kryvexis-logo.png" alt="Kryvexis" width={32} height={32} className="h-8 w-8 object-contain" style={{ filter: 'drop-shadow(0 10px 22px rgba(0,0,0,.35))' }} priority />
              <div>
                <div className="text-sm font-semibold tracking-tight">Kryvexis OS</div>
                <div className="text-[11px] leading-tight" style={{ color: 'rgba(255,255,255,0.62)' }}>{workspaceName ?? 'Workspace'}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <nav className={'px-3 pb-2 space-y-1'} style={{ color: 'rgba(255,255,255,0.92)' }}>
        {navMainItems.filter((it) => canSee(it.roles)).map((it) => {
          const on = pathname === it.href || pathname.startsWith(it.href + '/')
          return (
            <Link key={it.href} href={it.href} data-tour={`nav-${it.icon}`} title={it.label} className={'group relative flex items-center rounded-xl py-2 text-sm transition ' + (on ? 'bg-white/10' : 'hover:bg-white/5')}>
              {on && <span aria-hidden="true" className="absolute left-0 top-1/2 h-6 w-[3px] -translate-y-1/2 rounded-full" style={{ background: 'rgb(var(--kx-accent))' }} />}
              <span className={'ml-3'} style={{ color: on ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.74)' }}><NavIcon name={it.icon} /></span>
              <span className="ml-2 tracking-tight" style={{ color: on ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.88)' }}>{it.label}</span>
      ...
      <nav className={'px-3 pt-2 pb-3 space-y-1'} style={{ color: 'rgba(255,255,255,0.92)' }}>
        {navBottomItems.filter((it) => canSee(it.roles)).map((it) => {
          const on = pathname === it.href || pathname.startsWith(it.href + '/')
          return (
            <Link key={it.href} href={it.href} title={it.label} className={'group relative flex items-center rounded-xl py-2 text-sm transition ' + (on ? 'bg-white/10' : 'hover:bg-white/5')}>
              {on && <span aria-hidden="true" className="absolute left-0 top-1/2 h-6 w-[3px] -translate-y-1/2 rounded-full" style={{ background: 'rgb(var(--kx-accent))' }} />}
              <span className={'ml-3'} style={{ color: on ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.74)' }}><NavIcon name={it.icon} /></span>
              <span className="ml-2 tracking-tight" style={{ color: on ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.88)' }}>{it.label}</span>
              {on && <span className="ml-auto mr-2 h-1.5 w-1.5 rounded-full" style={{ background: 'rgb(var(--kx-accent))' }} />}
