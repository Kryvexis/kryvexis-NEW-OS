...
export default function ClientPicker({ clients }: { clients: ClientLite[] }) {
  ...
  const bucketed = React.useMemo(() => {
    ...
    for (const c of clients || []) (classifyClient(c) === "cash" ? cash : acc).push(c);
    const byName = (a: ClientLite, b: ClientLite) => (a.name || "").localeCompare(b.name || "");
    acc.sort(byName); cash.sort(byName);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="grid gap-1.5">
          ...
          <select
            ...
            value={bucket}
            onChange={(e) => { setBucket(e.target.value as any); setShowAll(false); setSelectedId(""); }}
          >
